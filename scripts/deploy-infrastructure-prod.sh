...
CERT_STACK_NAME=ProfoundBookClub-Certificate-prod
...
if [ -z "${CERTIFICATE_ARN}" ] && [ "${DEPLOY_WITHOUT_CERT:-}" != "1" ]; then
  ...
  if [ -z "${CERTIFICATE_ARN}" ] || [ "${CERTIFICATE_ARN}" == "None" ]; then
    ...
    echo "   ./scripts/deploy-prod-cert.sh"
    exit 1
  fi
...
pnpm exec cdk bootstrap aws://727508844146/us-east-2 --require-approval never

DEPLOY_ARGS=(ENVIRONMENT=prod)
...
if [ -n "${CERTIFICATE_ARN}" ]; then
  echo "🌐 Custom domain (after Route 53 alias records): https://profound-book-club.org"
else
