...
CERT_STACK_NAME=ProfoundBookClub-Certificate-dev
...
if [ -z "${CERTIFICATE_ARN}" ] && [ "${DEPLOY_WITHOUT_CERT:-}" != "1" ]; then
  ...
  if [ -z "${CERTIFICATE_ARN}" ] || [ "${CERTIFICATE_ARN}" == "None" ]; then
    ...
    echo "   ./scripts/deploy-dev-cert.sh"
    exit 1
  fi
...
if [ -n "${CERTIFICATE_ARN}" ]; then
  echo "🌐 Custom domain (after Route 53 alias records): https://dev.profound-book-club.org"
else
